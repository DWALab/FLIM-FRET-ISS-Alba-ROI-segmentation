function BinISSData(csv_path,output_path,untransfected_lifetime,untransfected_ratio,ch2_lower_thresh,ch2_upper_thresh)
    ...
    %if the data has shifted in comparison to the untransfected well
    %we use the first 10 points to estimate the shift in the A:D ratio
    
    untransfected_tau_well = untransfected_lifetime; 
    if isempty(untransfected_tau)
        sorted_ratio = sort(ratio);
        ratio_thresh = max(sorted_ratio(1:10)); 
        untransfected_tau_well = mean(tau(ratio <= ratio_thresh)); 
    else
